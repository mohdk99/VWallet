...
class _flight_ticket_scannerState extends State<flight_ticket_scanner> {
  ...
  void addItem(String code) async {
    ...
    String firstName = airline == 'OV'
        ? final_code.substring(
            final_code.indexOf('/') + 1, final_code.indexOf(' ') - 2)
        : final_code.substring(
            final_code.indexOf('/') + 1, final_code.indexOf(' '));
    ...
    insertFlightData(final_code, firstName, lastName, formatted, b_class,
  ...
  _qrCallback(String? code) {
    setState(() {
      ...
      if (keysList
          .where((element) =>
              flight_record_access.get(element)?.ticketCode == code)
          .isNotEmpty) {
