...
generate_code(int index) {
  ...
  String? item = '1234';
  if (index != null) {
    item = flight_record.get(index)?.ticketCode;
    // print(documentNumber[index]);

  } else if (index == null) {
    print('index is Null');
  } else if (flight_record.get(index)?.ticketCode == null) {
    dynamic? index = null;
  }
...
class _flightTicket_doc_listState extends State<flightTicket_doc_list> {
  ...
  delete_item(int index) {
    ...
    final removeItem = flight_record.get(index)?.airline;
    final removeItem3 = flight_record.get(index)?.createdDate;
    final removeItem4 = flight_record.get(index)?.dep;
    final removeItem5 = flight_record.get(index)?.dis;
    final removeItem6 = flight_record.get(index)?.firstName;
    final removeItem8 = flight_record.get(index)?.lastName;

    flight_record.deleteAt(index);

    listKey.currentState?.removeItem(
      index,
      (context, animation) => buildItem(removeItem, removeItem6!, removeItem8!,
          removeItem3!, removeItem4!, removeItem5!, index, animation),
    );
  ...
  Widget contentListView() {
    ...
    return ValueListenableBuilder(
        valueListenable: flight_record!.listenable(),
        builder: (context, Box<FlightTicket> data, _) {
        ...
        setState(() {
          ...
          createdDate.insert(index, list3);
            ...
            itemBuilder: (context, index, animation) => buildItem(
                flight_record.get(index)?.airline,
                flight_record.get(index)?.firstName,
                flight_record.get(index)?.lastName,
                flight_record.get(index)?.createdDate,
                flight_record.get(index)?.dep,
                flight_record.get(index)?.dis,
                /*flightTicket[index].airline,
          flightTicket[index].firstName,
          flightTicket[index].lastName,
          flightTicket[index].createdDate,
          flightTicket[index].dep,
          flightTicket[index].dis,*/
                index,
                animation),
          );
        });
